...
func NewGsrpcClient(endpoint, addressType string, key *signature.KeyringPair) (*GsrpcClient, error) {
	logrus.Info("Connecting to substrate chain with Gsrpc", "endpoint", endpoint)
...
func (gc *GsrpcClient) NewUnsignedExtrinsic(callMethod string, args ...interface{}) (interface{}, error) {
	logrus.Debug("Submitting substrate call...", "callMethod", callMethod, "addressType", gc.addressType, "sender", gc.key.Address)
	meta, err := gc.GetLatestMetadata()
	if err != nil {
		return nil, err
	}
...
func (gc *GsrpcClient) SignAndSubmitTx(ext interface{}) error {
	...
	logrus.Trace("Extrinsic submission succeeded")
...
func (gc *GsrpcClient) watchSubmission(sub *author.ExtrinsicStatusSubscription) error {
	for {
		...
		case err := <-sub.Err():
			logrus.Trace("Extrinsic subscription error", "err", err)
			return err
		}
	}
}
...
func (gc *GsrpcClient) BatchTransfer(receives []*Receive) error {
	...
	ext, err := gc.NewUnsignedExtrinsic(MethodBatch, calls)
	if err != nil {
		return err
	}
...
func (gc *GsrpcClient) AccountInfo(who []byte) (*types.AccountInfo, error) {
	ac := new(types.AccountInfo)
	exist, err := gc.QueryStorage(SystemModuleId, StorageAccount, who, nil, ac)
	if err != nil {
		return nil, err
	}
...
func (gc *GsrpcClient) NewVersionAccountInfo(who []byte) (*AccountInfo, error) {
	ac := new(AccountInfo)
	exist, err := gc.QueryStorage(SystemModuleId, StorageAccount, who, nil, ac)
	if err != nil {
		return nil, err
